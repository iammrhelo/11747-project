...
import torch
from data_utils import LetsGoCorpus, read_corpus_vocab, LetsGoDataLoader, data_iter
import sys
...
def test():
    ...
    x = LetsGoDataLoader(corpus.train)
    count = 0
    for src, tgt in data_iter(zip(x.src, x.tgt), 2):
        count += 1
        print(src)
        print(tgt)
        print("**************")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='./data/vocab.bin', type=str, help='output vocabulary file')

    args = parser.parse_args()
    #test()
    corpus = LetsGoCorpus('./data/union_data-1ab.p')
    train_src, train_tgt = corpus.get_train_sents()

    src_sents = read_corpus_vocab(train_src, source='src')
    tgt_sents = read_corpus_vocab(train_tgt, source='tgt')
